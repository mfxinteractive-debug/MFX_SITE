...
const Indoor = () => {
  ...
  const indoorData = productData["indoor-led"];
  
  // Create products array from JSON data
  const products = indoorData.models.map((model, index) => {
    // Get display specs
    const pixelPitch = model.specs.Display?.["Pixel Pitch"] || "P1.5";
    const brightness = model.specs.Display?.Brightness || "800-1200 nits";
    const refreshRate = model.specs.Display?.["Refresh Rate"] || "3840Hz";
    
    return {
      name: `${indoorData.name} - ${model.name}`,
      sizes: `${pixelPitch} Pixel Pitch | ${brightness} | ${refreshRate}`,
      desc: index === 0 ? indoorData.shortDescription : indoorData.fullDescription,
      images: [
    ...
    return (
      <div className="specs-modal-content">
        ...
        <div className="specs-features">
          <h4>Features</h4>
          <ul>
            {indoorData.features.map((feature, index) => (
        ...
        <div className="specs-applications">
          ...
          <div className="applications-tags">
            {indoorData.applications.map((app, index) => (
    ...
    <>
      ...
      <section className="px-section">
        <div className="px-container">
          <h2 className="section-title">{indoorData.name}</h2>
          <p className="section-subtitle">{indoorData.shortDescription}</p>
      ...
      <section className="px-cta">
        <div className="px-container">
          <h2>Ready for Indoor Digital Transformation?</h2>
          <p>
            Join hundreds of brands already using <strong>MULTIFX</strong> indoor LED displays for corporate, retail, and control room applications.
          </p>
