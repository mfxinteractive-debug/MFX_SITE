...
import ledData from "../../data/IndoorData.json";
...
const Indoor = () => {
  ...
  // Get indoor LED data from JSON
  useEffect(() => {
    const indoorLEDData = ledData["indoor-led"];
    if (indoorLEDData && indoorLEDData.products) {
      setProducts(indoorLEDData.products);
    }
  }, []);
  ...
  // Function to get display specs for LED
  const getDisplaySpecs = (product) => {
    const specs = [];
    
    // For LED displays, show pixel pitch
    if (product.specifications?.display?.pixel_pitch) {
      specs.push(product.specifications.display.pixel_pitch);
    }
    
    // Show size if available (for LED it might be in sizes or size)
    ...
    <>
      ...
      <section className="px-hero">
        <div className="px-hero-overlay">
          <h1>
            Premium <span className="px-highlight">Indoor LED Displays</span>
          </h1>
          <p className="px-hero-desc">
            High-resolution indoor LED displays with superior image quality, color accuracy, and energy efficiency for corporate, retail, and entertainment applications.
          </p>
        </div>
      </section>
      ...
      <section className="px-section">
        <div className="px-container">
          <h2 className="section-title">Indoor LED Display Solutions</h2>
          <p className="section-subtitle">From fine-pitch to large format displays for every indoor application</p>
      ...
      {/* CTA - Commented out as per TouchScreens */}
      {/* <section className="px-cta">
        <div className="px-container">
          <h2>Transform Your Space with Indoor LED</h2>
          <p>
            Enhance your corporate environment, retail space, or entertainment venue with our premium indoor LED display solutions.
          </p>
          <button className="px-main-cta">Request a Consultation</button>
        </div>
      </section> */}
    </>
  );
